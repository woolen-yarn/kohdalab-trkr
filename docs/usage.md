# KohdaLab TRKR Usage Guide

This guide keeps the detailed operating notes for KohdaLab TRKR. For first-time machine setup, see [Initial setup](initial_setup.md).

KohdaLab-TRKR is organized around a small public API for laboratory control.

For practical examples, see `api_usage.md`.

Project Structure
-----------------

![KohdaLab TRKR Project Structure](kohdalab-architecture.svg)

Measurement sequence diagrams:

- [Overview](measurement_sequences.md)
- [Signal Monitor](measurement-sequence-signal-monitor.svg)
- [TRKR](measurement-sequence-trkr.svg)
- [SRKR](measurement-sequence-srkr.svg)
- [STRKR](measurement-sequence-strkr.svg)
- [SRKR 2D](measurement-sequence-srkr-2d.svg)
...
実機確認 checklist は `hardware_smoke_test_ja.md` を使ってください。
...
Use `hardware_smoke_test.md` for the hardware verification checklist.
...
Use `windows_setup.md` when preparing a Windows instrument PC.
