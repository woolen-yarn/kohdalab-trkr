...
class GSC01:
    ...
    def ask(self, cmd: str) -> str:
        ...
        try:
            ...
            raise RuntimeError(f"{self.port} GSC01 returned non-ASCII data") from e
        if not response:
            raise TimeoutError(
                f"{self.port} GSC01 timed out waiting for {cmd} response"
            )
        return response

    def _exchange(self, cmd: str, *, expect_response: bool) -> bytes:
        if not self.is_connected():
            raise ConnectionError(f"{self.port} GSC01 serial connection is closed")
        ...
        try:
            ...
            raise ConnectionError(
                f"{self.port} GSC01 serial I/O failed for {cmd}: {e}"
            ) from e
    ...
    def is_ready(self) -> bool:
        ...
        if response not in {"R", "B"}:
            raise RuntimeError(f"Unexpected GSC01 ! response: {response!r}")
        return response == "R"
    ...
    def get_positions(self) -> list[int]:
        ...
        if len(fields) < self.axis_count:
            raise RuntimeError(f"Unexpected GSC01 Q response: {response!r}")
        positions: list[int] = []
    ...
    def wait_position(
        ...
        current = self.get_pos_raw(axis=axis)
        raise TimeoutError(
            f"{type(self).__name__} did not reach target: target={target_pos_raw}, current={current}"
        )
