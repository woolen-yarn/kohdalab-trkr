...
class Shot302GS:
    ...
    def get_pos_raw(self, axis: Optional[int] = None) -> int:
        ...
        if not positions:
            raise RuntimeError(f"Unexpected SHOT-302GS Q response: {self.get_status()}")
    ...
    def wait_position(
        ...
        while time.time() - t0 < timeout:
            ...
            time.sleep(poll_interval)
        current = self.get_pos_raw(axis=axis)
        raise TimeoutError(f"SHOT-302GS did not reach target: target={target_pos_raw}, current={current}")
